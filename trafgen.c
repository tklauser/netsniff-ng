...
struct ctx {
	bool rand, rfraw, jumbo_support, verbose, smoke_test, enforce, qdisc_path, rx_pump_back;
	size_t reserve_size;
	unsigned long num;
	unsigned int cpus;
	uid_t uid; gid_t gid;
...
static const char *short_options = "d:c:n:t:vJhS:rk:i:o:VRs:P:eE:pu:g:CHQqBD:b:";
static const struct option long_options[] = {
	...
	{"rx-pump-back",	no_argument,		NULL, 'B'},
...
static void __noreturn help(void)
{
	...
	puts("http://www.netsniff-ng.org\n\n"
	     ...
	     "  -B|--rx-pump-back              Pump back packets into RX path instead of sending\n"
		      ...
		      unsigned long orig_num)
{
	...
	if (ctx->rx_pump_back)
		set_sock_rx_pump_back(sock, ctx->verbose);
